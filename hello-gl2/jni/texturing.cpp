...
//    glBindTexture(GL_TEXTURE_2D, texture_id_);
    ...
    const model_desc m = model.frame(iframe);
    ...
    glDrawElements(GL_TRIANGLES, m.indices_size / 4, GL_UNSIGNED_INT, NULL);
...
bool load_model()
{
    ifstream file("/sdcard/infantry.md2", ios_base::in | ios_base::binary);
    if (!file.is_open())
        return false;
...
{
    cg_homework::init();
    cg_homework::resize(width, height);
    cg_homework::load_model();
}
