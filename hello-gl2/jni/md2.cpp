...
namespace cg_homework
{
    ...
    model_desc md2_model::frame(size_t n) const
    {
        model_desc m;
        m.verts = frames_[n];
        m.indices = indices_;
        m.texcoords = texcoords_;
        m.verts_size = verts_size_;
        m.texcoords_size = texcoords_size_;
        m.indices_size = indices_size_;
...
#pragma pack(pop)
    ...
    bool md2_model::load(istream &src)
    {
        ...
        auto load_indices = [&]() 
        {
            ...
            for (int n = 0; n < header.num_tris; ++n)
            {
                ...
                for (int i = 0; i < 3; ++i)
                {
                    ...
                    indices_buffer[n * 3 + i] = tri_desc.index_st[i];
                }
            }
        };
        ...
        {
            verts_size_ = frames_buffer[i].size() * sizeof(vertex);
            auto verts_ptr = &(frames_buffer[i][0]);
            glBindBuffer(GL_ARRAY_BUFFER, frames_[i]);
            glBufferData(GL_ARRAY_BUFFER, verts_size_, verts_ptr, GL_STATIC_DRAW);
        }

        texcoords_size_ = texcoords_buffer.size() * sizeof(texcoord);
        auto texcoords_ptr = &(texcoords_buffer[0]);
        glGenBuffers(1, &texcoords_);
        glBindBuffer(GL_ARRAY_BUFFER, texcoords_);
        glBufferData(GL_ARRAY_BUFFER, texcoords_size_, texcoords_ptr, GL_STATIC_DRAW);

        indices_size_ = indices_buffer.size() * sizeof(int);
        auto indices_ptr = &(indices_buffer[0]);
        glGenBuffers(1, &indices_);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indices_);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices_size_, indices_ptr, GL_STATIC_DRAW);
